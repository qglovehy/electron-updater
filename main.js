...
const winUpdateConfig = {
  ...
  updateFilePath: '', //远程包路径
  localUpdateVersionFilePath: production === 'dev' ? `${rootPath}/latest` :`${rootPath}/resources/latest`, //本地版本信息路径
  localUpdateFilePath: production === 'dev' ? `${rootPath}/public` : `${rootPath}/resources`, //本地包路径
  updateSteps: [
...
function createWindow () {
  ...
  ipcMain.on('window-download-newfile', function (event) {
    ...
    http
    .get(winUpdateConfig.updateFilePath, (response) => {
    ...
    file?.on('finish', function () {
      ...
      asar
      .createPackage(sourceDir, destPath)
       .then(() => {
